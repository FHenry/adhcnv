 ...
 */
class pdf_formationlight extends ModelePDFPropales
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->propal->multidir_output[$conf->entity])
		{
			...
			if (file_exists($dir))
			{
				...
				// Ajouter le fichier de page de garde
				// $file_FirstPAge = DOL_DATA_ROOT .'/ecm/docPDF/PageDeGarde/pagedegarde.pdf';
				// $pagecusto=0;
				// if (file_exists($file_FirstPAge) && is_readable($file_FirstPAge)) {
				// 	$pagecounttmp = $pdf->setSourceFile($file_FirstPAge);
				// 	if ($pagecounttmp)
				// 	{
				// 		for ($i = 1; $i <= $pagecounttmp; $i++)
				// 		{
				// 			$tplidx_FirstPage = $pdf->ImportPage($i);
				// 			$s = $pdf->getTemplatesize($tplidx_FirstPage);
				// 			$pdf->AddPage($s['h'] > $s['w'] ? 'P' : 'L');
				// 			$pdf->useTemplate($tplidx_FirstPage);
				// 			$pagenb++;
				// 			$pagecusto=$pagecounttmp;
				// 		}
				// 	}
				// }
				...
				if ($notetoshow)
				{
					...
					$notetoshow.= '<br/>';
					$notetoshow.= $conf->global->PROPOSAL_ACCESS42_BDC;
					$notetoshow.= '<br/><br/>';
					$notetoshow.= $conf->global->PROPOSAL_ACCESS42_FORMATION_TVA;
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		...
		//return pdf_pagefoot($pdf, $outputlangs, 'PROPOSAL_ACCESS42_FORMATION', $this->emetteur, $this->marge_basse, $this->marge_gauche, $this->page_hauteur, $object, $showdetails, $hidefreetext);
		$pdf->SetFont('', '', 7);
		$pdf->SetDrawColor(224, 224, 224);
		$footertext=$conf->global->PROPOSAL_ACCESS42_FORMATION;

		$dims=$pdf->getPageDimensions();
		$freetextheight=pdfGetHeightForHtmlContent($pdf, dol_htmlentitiesbr($footertext), 1, 'UTF-8', 0);
		$marginwithfooter=$this->marge_basse + $freetextheight;
		$posy=$marginwithfooter;
		$pdf->SetY(-$posy);
		$pdf->line($dims['lm'], $dims['hk'], $dims['wk']-$dims['rm'], $dims['hk']);
		$posy--;
		$pdf->SetFont('', '', 7);
		$pdf->SetXY($dims['lm'], -$posy);
		$pdf->writeHTMLCell($dims['wk']-$dims['rm']-$dims['lm'], 2, $dims['lm'],-$posy,$outputlangs->convToOutputCharset($footertext), 0, 1,0, false, true);
	...
	protected function _signature_area(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$pdf->MultiCell($largcol, $tab_hl*3, $conf->global->PROPOSAL_ACCSES42_FORMATION_SIGNATURE, 0, 'L');
